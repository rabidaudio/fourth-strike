...
```bash
ssh -i fourth-strike-aws.pem ubuntu@app.fourth-strike.com 'source ./env && cd fourth-strike && git pull && docker compose pull && docker compose down --remove-orphans && docker compose run app bundle exec rake db:migrate:with_data && docker compose run app bundle exec whenever --update-crontab && docker compose up -d'
```
